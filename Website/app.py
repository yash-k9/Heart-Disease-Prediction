...
from loginform import Loginf, Reginf, ForgetPwd
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if db.session.query(exists().where(User.username == form.uname.data)).scalar():
            flash('User already taken! Are you registered already?', 'danger')
        elif db.session.query(exists().where(User.email == form.email.data)).scalar():
            flash('Email already taken! Are you registered already?',  'danger')
        else:
    ...
    return render_template("registration.html", form=form, name = "Register")
...
def predict():
    ...
    if form.validate_on_submit() or request.method == 'POST':
        ...
        return render_template("result.html", form = form, val = val, name = "Result", acc = "83%")
...
def forget_password():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        user = User.query.filter_by(username=form.uname.data).first_or_404()
        if(user.email == form.email.data):
            return render_template('forgetPassword.html', user=user, form = form, name = "Forget Password")
        else:
            flash('Invalid Username or Email', 'danger')
            #return render_template('forgetPassword.html', form = form, name = "Forget Password", message = "Invalid User or Password")
        
    return render_template("forgetPassword.html", form = form, name = "Password Reset")
